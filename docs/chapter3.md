...
**Tips：**整数常量是无符号的，需要通过uint<W>直接标识整数的位宽，W是正整数。
...
**读取一个尚未初始化的变量的值会得到一个未定义的结果，编译器会检测并报警告。没有显示初始化的变量是未初始化的。**
...
无符号整数和有符号整数具有任意宽度，以位表示。宽度为W的无符号整数声明为：uint<W>，宽度为W的有符号整数声明为：int<W>。W必须是为大于0的正整数。
...
枚举值通常与整数常量存在明确的数值映射关系，在**类型兼容且语言语法允许的上下文**中，可在整数常量适用的绝大多数场景下使用。
